...
| `JobAssignment` | `job_id` (ref. Rider App), `professional_id`, `status` (`pending` / `accepted` / `in_progress`, `completed`), `accepted_at`             |

> `job_id` es una **referencia externa**: Driver App no es dueña del Job, solo guarda el ID para asociar asignaciones y luego actualizar el estado en Rider App.
...
| `Job` | `id`, `client_id` (Clerk), `service_type` (`plomeria` / `electricidad` / `gas`), `description`, `location` (lat/lng), `requested_date`, `urgency` (`immediate` / `scheduled`), `status` (`pending` / `accepted` / `completed` / `cancelled`), `professional_id` (ref. Driver App), `estimated_price` |
